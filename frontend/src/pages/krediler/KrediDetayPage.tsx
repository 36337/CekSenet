// ============================================
// ÇekSenet - Kredi Detay Sayfası
// Kredi bilgileri, taksit tablosu ve işlemler
// ============================================
...
export function KrediDetayPage() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const location = useLocation()
  const { user } = useAuth()
  const krediId = Number(id)
  ...
  const handleTaksitOde = async () => {
    ...
    try {
      await taksitOde(krediId, selectedTaksit.id, {
  ...
  const handleErkenOdeme = async () => {
    ...
    try {
      const result = await erkenOdeme(krediId, {
      ...
      setSuccessMessage(`Erken ödeme tamamlandı. ${result.odenen_taksit_sayisi} taksit ödendi.`)
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteKredi(krediId)
      navigate('/krediler', {
        state: { message: 'Kredi başarıyla silindi' }
      })
    } catch (err) {
  ...
  const getTaksitRowClass = (durum: TaksitDurum): string => {
    switch (durum) {
      case 'odendi':
        return 'bg-green-50'
      case 'gecikti':
  ...
  if (error || !kredi) {
    return (
      <div className="space-y-4">
        <Button outline onClick={() => navigate('/krediler')}>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <Heading>{kredi.banka_adi || 'Banka Belirtilmemiş'}</Heading>
              <Badge color={KREDI_DURUM_COLORS[kredi.durum] as any}>
                {KREDI_DURUM_LABELS[kredi.durum]}
              </Badge>
            </div>
            <Text className="mt-1">
              {KREDI_TURU_LABELS[kredi.kredi_turu]} • {formatKrediTutar(kredi.anapara, kredi.para_birimi)}
            </Text>
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Button outline onClick={() => navigate(`/krediler/${krediId}/duzenle`)}>
            <PencilSquareIcon className="h-5 w-5" />
            Düzenle
          </Button>
          {isAdmin && (
            <Button color="red" outline onClick={() => setIsDeleteDialogOpen(true)}>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          title="Anapara"
          value={formatKrediTutar(kredi.anapara, kredi.para_birimi)}
          icon={<BanknotesIcon className="h-5 w-5" />}
          color="blue"
        />
        <StatCard
          title="Toplam Ödeme"
          value={formatKrediTutar(kredi.toplam_odeme, kredi.para_birimi)}
          icon={<CurrencyDollarIcon className="h-5 w-5" />}
          color="default"
        />
        <StatCard
          title="Kalan Borç"
          value={formatKrediTutar(kredi.ozet.kalan_borc, kredi.para_birimi)}
          icon={<CalendarDaysIcon className="h-5 w-5" />}
          color={kredi.ozet.kalan_borc > 0 ? 'amber' : 'green'}
        />
        <StatCard
          title="Ödenen Tutar"
          value={formatKrediTutar(kredi.ozet.odenen_tutar, kredi.para_birimi)}
      ...
      <div className="rounded-lg border border-zinc-200 bg-white p-6">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
          ...
          <div>
            ...
            <Text className="font-medium">{formatKrediTarih(getBitisTarihi(kredi.baslangic_tarihi, kredi.vade_ay))}</Text>
          </div>
          <div>
            <Text className="text-xs text-zinc-500">Toplam Faiz</Text>
            <Text className="font-medium">{formatKrediTutar(kredi.toplam_odeme - kredi.anapara, kredi.para_birimi)}</Text>
          </div>
        </div>
        ...
        {kredi.ozet.geciken_taksit > 0 && (
          <div className="mt-4 flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 p-3 text-red-700">
            ...
            <Text className="text-sm">
              ...
              Toplam: {formatKrediTutar(kredi.ozet.geciken_tutar, kredi.para_birimi)}
            </Text>
          </div>
        )}
            ...
            <TableBody>
              {kredi.taksitler.map((taksit) => (
                <TableRow key={taksit.id} className={getTaksitRowClass(taksit.durum)}>
                  ...
                  <TableCell>{formatKrediTarih(taksit.vade_tarihi)}</TableCell>
                  <TableCell className="font-medium">
                    {formatKrediTutar(taksit.tutar, kredi.para_birimi)}
                  </TableCell>
                  <TableCell>
                    <Badge color={TAKSIT_DURUM_COLORS[taksit.durum] as any}>
                      {TAKSIT_DURUM_LABELS[taksit.durum]}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    {taksit.odeme_tarihi ? formatKrediTarih(taksit.odeme_tarihi) : '-'}
                  </TableCell>
                  <TableCell>
                    {taksit.odenen_tutar ? formatKrediTutar(taksit.odenen_tutar, kredi.para_birimi) : '-'}
                  </TableCell>
              ...
              <div className="rounded-lg bg-zinc-50 p-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Text className="font-medium">{formatKrediTarih(selectedTaksit.vade_tarihi)}</Text>
                  </div>
                  <div>
                    <Text className="text-xs text-zinc-500">Taksit Tutarı</Text>
                    <Text className="font-medium">
                      {formatKrediTutar(selectedTaksit.tutar, kredi?.para_birimi || 'TRY')}
                    </Text>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-4">
                ...
                <Text className="mt-2 text-sm text-amber-700">
                  ...
                  Toplam: <strong>{formatKrediTutar(kredi.ozet.kalan_borc, kredi.para_birimi)}</strong>
                </Text>
              </div>
      ...
      <Dialog open={isDeleteDialogOpen} onClose={() => setIsDeleteDialogOpen(false)}>
        ...
        <DialogBody>
          <Text className="text-sm text-zinc-600">
            <strong>{kredi.banka_adi || 'Bu kredi'}</strong> - {KREDI_TURU_LABELS[kredi.kredi_turu]} kaydını silmek istediğinizden emin misiniz?
          </Text>
        </DialogBody>
        <DialogActions>
